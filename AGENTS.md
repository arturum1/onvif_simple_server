...
- Focus (imaging service) and SendAuxiliaryCommand toggles are entirely config-driven: gated by `imaging=1` (imaging), `aux_command`/`aux_exec` key pairs (PTZ aux commands, capped at `MAX_AUX_COMMANDS`, raised from upstream's 8 to 12 because the web panel is a generic front end and spare channels are worth advertising; the binding limit is the `MAX_CAT_LEN` line `GetNode` expands the block into). Focus commands accept `%f`; `focus_set_auto_focus` / `ir_cut_filter_set` accept `%s`. Read-back uses `spawn_capture`. Aux command strings are validated against shell metachars before use (`is_safe_aux_command`). Aux commands are advertised in GetNode (`%AUX_COMMANDS%`, added to both zoom and nozoom variants).
